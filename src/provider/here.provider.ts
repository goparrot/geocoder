...
export class HereProvider extends AbstractHttpProvider {
    constructor(httpClient: AxiosInstance, private readonly appId: string, private readonly appCode: string, private readonly cit: boolean = false) {
        super(httpClient);
    ...
    get geocodeUrl(): string {
        return `https://geocoder.${this.cit ? 'cit.' : ''}api.here.com/6.2/geocode.json`;
    }
    ...
    get reverseUrl(): string {
        return `https://reverse.geocoder.${this.cit ? 'cit.' : ''}api.here.com/6.2/reversegeocode.json`;
    }
